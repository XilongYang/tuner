...
await page.evaluate(async () => {
  ...
  const sentence = {
    id: crypto.randomUUID(), text: 'Alpha beta', lang: 'en', hidden: true, recorder: new Recorder(),
...
const result = await page.evaluate(() => {
  ...
  const el = speaking[0];
  const afterColor = el ? getComputedStyle(el, '::after').color : null;
  const rowIsHidden = document.querySelector('.sentence-row').classList.contains('is-hidden');
  return { speakingCount: speaking.length, afterColor, rowIsHidden };
});
